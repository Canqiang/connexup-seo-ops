...
export function ReviewSignalPanel({ merchantId, artifacts, loading, error }: { merchantId: string; artifacts: SpecialistArtifactWire[]; loading: boolean; error?: unknown }) {
  ...
  return <section aria-label="复盘信号" className="data-panel review-signal-panel">
    ...
    {error ? <div className="page-state compact is-error" role="alert">复盘信号读取失败。</div> : loading ? <div className="page-state compact" role="status">读取复盘…</div> : latest ? <>
