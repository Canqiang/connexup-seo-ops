...
test("submits credentials without persisting the password and returns to a safe route", async () => {
  ...
  await user.type(screen.getByLabelText("工作邮箱"), "operator@example.com");
...
test("shows a generic credential error", async () => {
  const user = userEvent.setup();
  vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(JSON.stringify({ message: "wrong password" }), {
