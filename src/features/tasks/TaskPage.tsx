...
export function TaskPage() {
  ...
  const readback = (next: SeoTask) => { setTask(next); taskResource.reload(); eventResource.reload(); };
  if (taskResource.loading && !task) return <div className="page-state" role="status">正在读取任务聚合…</div>;
  if (taskResource.error || !task) return <div className="page-state is-error" role="alert">任务不存在、不可见或读取失败。</div>;
  const canManage = hasPermission(user?.permissions, "seoops.manage");
  return <>
    <header className="page-heading task-heading"><div><span className="eyebrow">TASK / {task.id}</span><h1>{task.title}</h1><p>{task.merchant_name} → {task.location_name ?? "商户级"} · {task.task_type}</p></div><div className="heading-actions"><button className="icon-button" aria-label="刷新任务" onClick={taskResource.reload} type="button"><RefreshCw size={16} /></button>{canManage && task.status !== "APPROVED" ? <button className="secondary-button" onClick={() => setShowRevision((value) => !value)} type="button">新建修订</button> : null}{canManage && task.status !== "APPROVED" ? <button className="primary-button" onClick={() => setShowEvidence((value) => !value)} type="button"><Plus size={15} /> 附加证据</button> : null}</div></header>
    <section className="task-state-ribbon"><div><span>任务版本</span><strong>rev {task.task_revision}</strong></div><div><span>状态版本</span><strong>{task.state_version}</strong></div><div><span>执行状态</span><strong>{task.status}</strong></div><div><span>证据状态</span><strong>{task.evidence_state}</strong></div><div><span>影响</span><strong>{task.impact}</strong></div></section>
