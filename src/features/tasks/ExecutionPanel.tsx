...
export function ExecutionPanel({ task, onReadback }: {
  ...
  return <section className="data-panel execution-panel">
    ...
    <div className="execution-body">
      ...
      {task.status === "APPROVED" && mode !== "READ_ONLY" ? <Gate2Block busy={busy} onConfirm={() => void confirm()} task={task} /> : null}
      ...
      {task.status === "OUTCOME_UNKNOWN" ? <div className="exec-unknown">
        ...
        <button className="danger-button" onClick={() => setResolving(true)} type="button">开始查证</button>
      </div> : null}
    ...
    {resolving ? <OutcomeDialog task={task} onClose={() => setResolving(false)} onDone={() => { setResolving(false); void seoOpsApi.task(task.id).then(onReadback); }} /> : null}
  </section>;
}
...
function Gate2Block({ task, busy, onConfirm }: { task: SeoTask; busy: boolean; onConfirm: () => void }) {
  ...
  return <div className="gate2-block">
    ...
    {preview.loading ? <p className="quiet-copy">校验中…</p> : <ul className="gate-checks">
      {checks.map((c) => <li className={c.passed ? "is-pass" : "is-fail"} key={c.key}>
    ...
    <button className="primary-button" disabled={busy || !preview.data?.confirmable} onClick={onConfirm} type="button">
      <PlayCircle size={14} /> {busy ? "确认中…" : `确认执行（attempt #${task.attempt_count + 1}）`}
    </button>
