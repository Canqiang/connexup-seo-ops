...
export function resolveTaskDraftDisplay(task: SeoTask, drafts: DraftWire[]) {
  if (task.task_type !== "GBP_POST") {
    return { displayedDraft: drafts.at(-1), candidateDraft: undefined, approvalIdentityValid: true };
  }
  ...
  return { displayedDraft, candidateDraft, approvalIdentityValid: Boolean(displayedDraft) };
}

export function TaskDecisionHero({ task, decision, displayedDraft, candidateDraft, artifacts = [], children }: {
  task: SeoTask;
  ...
  const renderDraft = (draft: DraftWire, label: string) => <><strong>{label} v{draft.version}</strong><p>{draft.body}</p>
  ...
  return <section aria-label="当前决策" className="task-decision-hero">
    ...
    <section aria-label="当前内容" className="task-current-content"><span className="eyebrow">当前内容 / 成品</span>
      {displayedDraft ? renderDraft(displayedDraft, task.task_type === "GBP_POST" ? "当前批准对象" : "当前稿件")
