...
export function InboxPage() {
  ...
  return <>
    ...
    <section className="data-panel">
      <div className="panel-heading"><InboxFilters filters={filters} merchant={workspace.merchant} onChange={change} /><span className="result-count">{resource.data?.total ?? "—"} 项</span></div>
      ...
      {resource.data ? <div className="table-wrap"><table className="task-table"><thead><tr><th>优先级</th><th>任务 / 商户</th><th>地点</th><th>影响</th><th>状态</th><th>证据</th><th>负责人</th><th>到期</th><th>更新</th><th /></tr></thead>
        <tbody>{resource.data.items.map((task) => <tr key={task.id}>
          ...
          <td><button className="table-link" onClick={() => navigate(`/tasks/${task.id}`)} type="button"><strong>{task.title}</strong><small>{task.merchant_name} · rev {task.task_revision}</small></button></td>
          <td>{task.location_name ?? "商户级"}</td><td>{task.impact}</td><td><span className={`status-pill is-${task.status.toLocaleLowerCase()}`}>{task.status}</span></td>
          <td>{task.evidence_state}</td><td>{task.owner_id ?? "未分配"}</td><td>{formatDate(task.due_at)}</td><td>{formatDate(task.updated_at)}</td>
          <td><button aria-label={`打开任务 ${task.title}`} className="row-arrow" onClick={() => navigate(`/tasks/${task.id}`)} type="button"><ArrowRight size={15} /></button></td>
        </tr>)}</tbody></table>
        {!resource.data.items.length ? <div className="empty-state"><h2>当前筛选没有任务</h2><p>调整筛选条件，或创建新的执行任务。</p></div> : null}
        <div className="pagination"><button disabled={filters.offset === 0} onClick={() => change({ offset: Math.max(0, (filters.offset ?? 0) - 50) })}>上一页</button><span>{(filters.offset ?? 0) + 1}–{Math.min((filters.offset ?? 0) + 50, resource.data.total)} / {resource.data.total}</span><button disabled={(filters.offset ?? 0) + 50 >= resource.data.total} onClick={() => change({ offset: (filters.offset ?? 0) + 50 })}>下一页</button></div>
      </div> : null}
    </section>
