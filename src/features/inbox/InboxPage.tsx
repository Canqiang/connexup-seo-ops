...
import { evidenceStateLabel } from "../../app/statusCopy";
    ...
    {tab === "proposals" ? <section className="data-panel"><ProposalsTab merchantId={filters.merchant_id} /></section> : <section className="data-panel">
      ...
      {resource.data ? <div className="table-wrap"><table className="task-table"><thead><tr><th>优先级</th><th>任务 / 商户</th><th>地点</th><th>影响</th><th>状态</th><th>证据</th><th>负责人</th><th>到期</th><th>更新</th><th /></tr></thead>
        <tbody>{items.map((task) => {
          ...
          <td>{evidenceStateLabel(task.evidence_state)}</td><td>{formatTaskOwner(task.owner_id)}</td><td>{formatDate(task.due_at)}</td><td>{formatDate(task.updated_at)}</td>
