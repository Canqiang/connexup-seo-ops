...
def _merchant_lifecycle_token(
    conn: sqlite3.Connection, merchant_id: int
) -> tuple[int, str]:
    row = conn.execute(
        "SELECT generation,content_sha256 FROM merchant_status_events "
        "WHERE merchant_id=? ORDER BY generation DESC LIMIT 1",
        (merchant_id,),
    ).fetchone()
    if row is None:
        raise HTTPException(status_code=409, detail="merchant lifecycle history is missing")
    return int(row["generation"]), str(row["content_sha256"])
...
_TASK_SELECT = (
    ...
    "p.source_kind AS plan_source_kind, "
...
def patch_task(
    ...
    try:
        task = fetch_task(conn, task_id)
        fetch_active_merchant(conn, int(task["merchant_id"]))
...
def _finish_failed_dispatch(
    ...
    *,
    ...
    merchant_lifecycle: tuple[int, str],
    ...
    try:
        ...
        if (
            ...
            raise HTTPException(
                status_code=409, detail="task preparation lifecycle changed"
            )
        finished_at = now_iso()
...
def execute_task(
    ...
    try:
        task = fetch_task(conn, task_id)
        merchant = fetch_active_merchant(conn, int(task["merchant_id"]))
        merchant_lifecycle = _merchant_lifecycle_token(
            conn, int(task["merchant_id"])
        )
        ...
        claimed_task_version = body.expected_version + 1
        append_task_event(
            conn,
            entity_type="TASK",
            entity_id=task_id,
    ...
    try:
        ...
        failed = _finish_failed_dispatch(
            ...
            expected_task_version=claimed_task_version,
    ...
    try:
        ...
        fetch_active_merchant(conn, int(task["merchant_id"]))
        if (
            task["version"] != claimed_task_version
            or _merchant_lifecycle_token(conn, int(task["merchant_id"]))
            != merchant_lifecycle
        ):
            raise HTTPException(
