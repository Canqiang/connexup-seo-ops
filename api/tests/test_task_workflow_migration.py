...
def _install_performance_0001_over_legacy_tasks(path) -> str:
    from app.migrations import PERFORMANCE_MIGRATION, apply_migrations
    ...
    try:
        ...
        row = conn.execute(
            ...
            "WHERE name='guard_merchants_delete_with_durable_history'"
        ).fetchone()
        assert row is not None
        assert row[0:2] == ("trigger", "merchants")
        return str(row[2])
    finally:
        conn.close()
...
def test_performance_first_legacy_tasks_restore_delete_guard_and_preserve_logical_count(
    tmp_path, monkeypatch
):
    path = tmp_path / "performance-first-legacy-tasks.db"
    _seed_legacy_database(path)
    guard_before = _install_performance_0001_over_legacy_tasks(path)
    ...
    conn = connect()
    try:
        guard_after = conn.execute(
            "SELECT sql FROM sqlite_master "
            "WHERE type='trigger' "
            "AND name='guard_merchants_delete_with_durable_history'"
        ).fetchone()[0]
        materialized = conn.execute("SELECT COUNT(*) FROM tasks").fetchone()[0]
        draft_candidates = sum(
            len(json.loads(row[0])["tasks"])
            for row in conn.execute(
                "SELECT payload_json FROM task_plan_revisions "
                "WHERE decision_state='DRAFT'"
            )
        )
        assert guard_after == guard_before
        assert (materialized, draft_candidates, materialized + draft_candidates) == (
            7,
            2,
            9,
        )
...
def test_0002_hook_failure_rolls_back_and_restores_performance_delete_guard(
    tmp_path, monkeypatch
):
    from app import task_migrations

    path = tmp_path / "performance-guard-hook-failure.db"
    _seed_legacy_database(path)
    guard_before = _install_performance_0001_over_legacy_tasks(path)
    conn = sqlite3.connect(path)
    conn.row_factory = sqlite3.Row
    ...
    def fail_conversion(_conn):
        raise RuntimeError("injected 0002 conversion failure")

    monkeypatch.setattr(
        task_migrations, "_convert_legacy_runs_and_tasks", fail_conversion
    )
    with pytest.raises(RuntimeError, match="injected 0002 conversion failure"):
    ...
    assert conn.execute(
        ...
        "WHERE type='table' AND name='task_plans'"
    ).fetchone()[0] == 0
    assert conn.execute("PRAGMA foreign_key_check").fetchall() == []
...
def _seed_interim_formal_graph(conn):
    ...
    items = [
        {
            "depends_on": [],
            "expected_outcome": "Interim source outcome",
            "key": "interim-source",
            "parameters": {"category": None, "description": None},
            "rationale": "Interim source rationale",
            "scheduled_start": None,
            "task_type": "PREPARE_ONLY",
            "title": "Interim Source",
        },
        {
            "depends_on": ["interim-source"],
            "expected_outcome": "Interim replacement outcome",
            "key": "interim-replacement",
            "parameters": {"category": None, "description": None},
            "rationale": "Interim replacement rationale",
            "scheduled_start": None,
            "task_type": "PREPARE_ONLY",
            "title": "Interim Replacement",
        },
    ]
    payload = json.dumps(
        {"schema_version": "seo_ops.task_plan.v1", "tasks": items},
        sort_keys=True,
        separators=(",", ":"),
    )
    ...
    source_definition_checksum = hashlib.sha256(
        json.dumps(items[0], sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    replacement_definition_checksum = hashlib.sha256(
        json.dumps(items[1], sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
    parameters_json = '{"category":null,"description":null}'
    conn.execute(
        "INSERT INTO tasks "
        "(id, merchant_id, plan_id, plan_revision, task_key, task_type, workflow_version, "
        "parameters_json, definition_checksum, title, rationale, expected_outcome, "
        "status, version, created_at) "
        "VALUES (41, 1, ?, 1, 'interim-source', 'PREPARE_ONLY', 1, ?, ?, "
        "'Interim Source', 'Interim source rationale', 'Interim source outcome', "
        "'NEEDS_ATTENTION', 7, '2026-09-03T00:00:00+00:00')",
        (plan_id, parameters_json, source_definition_checksum),
    )
    conn.execute(
        "INSERT INTO tasks "
        "(id, merchant_id, plan_id, plan_revision, task_key, task_type, workflow_version, "
        "parameters_json, definition_checksum, title, rationale, expected_outcome, "
        "status, version, replaces_task_id, "
        "created_at) VALUES (42, 1, ?, 1, 'interim-replacement', 'PREPARE_ONLY', 1, "
        "?, ?, 'Interim Replacement', 'Interim replacement rationale', "
        "'Interim replacement outcome', 'PENDING', 3, 41, "
        "'2026-09-03T00:01:00+00:00')",
        (plan_id, parameters_json, replacement_definition_checksum),
    )
...
def test_states_v2_rebuild_failure_restores_interim_schema(tmp_path, monkeypatch):
    ...
    apply_migrations(
        conn,
        python_migrations=task_migrations.task_workflow_python_migrations(),
...
def test_unapproved_non_pristine_task_is_preserved_outside_draft_plan(
    tmp_path, monkeypatch
):
    ...
    conn = sqlite3.connect(path)
    conn.row_factory = sqlite3.Row
    migrated = conn.execute("SELECT * FROM tasks WHERE id=20").fetchone()
    assert migrated is not None
    assert migrated["source_run_id"] == 20
    assert migrated["status"] == "PREPARING"
    migration_plan = conn.execute(
        "SELECT * FROM task_plans WHERE id=?", (migrated["plan_id"],)
    ).fetchone()
    assert migration_plan["source_kind"] == "MIGRATION"
    assert migration_plan["source_run_id"] is None
    assert migration_plan["approved_revision"] == 1
    draft = conn.execute(
        "SELECT r.payload_json FROM task_plans p "
        "JOIN task_plan_revisions r ON r.plan_id=p.id "
        "WHERE p.source_run_id=20 AND r.decision_state='DRAFT'"
    ).fetchone()
    assert [item["key"] for item in json.loads(draft[0])["tasks"]] == [
        "draft-second"
    ]
...
def test_unapproved_todo_with_operator_evidence_is_preserved_as_non_pristine(
    tmp_path, monkeypatch
):
    path = tmp_path / "unapproved-operator-evidence.db"
    _seed_legacy_database(path)
    conn = sqlite3.connect(path)
    conn.execute(
        "UPDATE tasks SET evidence_note=? WHERE id=20",
        ("Operator verified the existing citation by phone.",),
    )
    conn.commit()
    conn.close()
    monkeypatch.setenv("SEO_OPS_DB", str(path))
    ...
    assert migrated["evidence_note"] == (
        "Operator verified the existing citation by phone."
    )
    migration_plan = conn.execute(
        "SELECT source_kind,source_run_id,approved_revision "
        "FROM task_plans WHERE id=?",
        (migrated["plan_id"],),
    ).fetchone()
    assert tuple(migration_plan) == ("MIGRATION", None, 1)
    draft = conn.execute(
        "SELECT r.payload_json FROM task_plans p "
        "JOIN task_plan_revisions r ON r.plan_id=p.id "
        "WHERE p.source_run_id=20 AND r.decision_state='DRAFT'"
...
def test_unapproved_todo_with_completed_at_uses_non_pristine_preservation_path(
    tmp_path, monkeypatch
):
    path = tmp_path / "unapproved-completed-at.db"
    _seed_legacy_database(path)
    conn = sqlite3.connect(path)
    conn.execute(
        "UPDATE tasks SET completed_at=? WHERE id=20",
    ...
    assert migrated is not None
    assert migrated["status"] == "PENDING"
    assert migrated["updated_at"] == "2026-08-07T00:00:00+00:00"
    migration_plan = conn.execute(
        "SELECT source_kind,source_run_id,approved_revision "
        "FROM task_plans WHERE id=?",
        (migrated["plan_id"],),
...
def test_unapproved_uppercase_todo_remains_a_draft_instead_of_becoming_actionable(
    tmp_path, monkeypatch
):
    path = tmp_path / "uppercase-todo.db"
    _seed_legacy_database(path)
    conn = sqlite3.connect(path)
    conn.execute("PRAGMA ignore_check_constraints = ON")
    conn.execute("UPDATE tasks SET status='TODO' WHERE id=20")
    conn.commit()
    conn.close()
    monkeypatch.setenv("SEO_OPS_DB", str(path))
    from app.db import init_db
    ...
    conn = sqlite3.connect(path)
    assert conn.execute("SELECT 1 FROM tasks WHERE id=20").fetchone() is None
    draft = conn.execute(
        "SELECT r.payload_json FROM task_plans p "
        "JOIN task_plan_revisions r ON r.plan_id=p.id "
        "WHERE p.source_run_id=20 AND r.decision_state='DRAFT'"
...
def test_unapproved_all_non_pristine_tasks_are_preserved_without_empty_draft(
    tmp_path, monkeypatch
):
    path = tmp_path / "all-non-pristine.db"
    _seed_legacy_database(path)
    conn = sqlite3.connect(path)
    conn.execute("UPDATE tasks SET status='cancelled' WHERE source_run_id=20")
    conn.commit()
    conn.close()
    monkeypatch.setenv("SEO_OPS_DB", str(path))
    from app.db import init_db
    ...
    conn = sqlite3.connect(path)
    assert conn.execute(
        "SELECT COUNT(*) FROM task_plans WHERE source_run_id=20"
    ).fetchone()[0] == 0
    assert conn.execute(
        "SELECT COUNT(*) FROM tasks WHERE source_run_id=20 AND status='CANCELLED'"
    ).fetchone()[0] == 2
    assert conn.execute(
        "SELECT COUNT(DISTINCT plan_id) FROM tasks WHERE source_run_id=20"
    ).fetchone()[0] == 1
    assert conn.execute("PRAGMA foreign_key_check").fetchall() == []
    conn.close()


def test_legacy_runs_receive_fail_closed_dispatch_state_and_unique_tokens(
    tmp_path, monkeypatch
):
    path = tmp_path / "legacy-run-dispatch.db"
    _seed_legacy_database(path)
    conn = sqlite3.connect(path)
    conn.executemany(
        "INSERT INTO runs(id,merchant_id,coreai_run_id,status,trigger_kind,"
        "plan_approved_at,created_at,finished_at) "
        "VALUES (?,1,?,?, 'manual',NULL,'2026-09-04T00:00:00+00:00',NULL)",
        [
            (30, "legacy-running-id", "running"),
            (31, "legacy-failed-id", "failed"),
            (32, "legacy-succeeded-id", "succeeded"),
        ],
    )
    conn.commit()
    conn.close()
    ...
    conn = sqlite3.connect(path)
    assert conn.execute(
        "SELECT id,dispatch_state,source_agent_id "
        "FROM runs WHERE id>=30 ORDER BY id"
    ).fetchall() == [
        (30, "UNKNOWN", None),
        (31, "FAILED", None),
        (32, "DISPATCHED", None),
    ]
    conn.execute("UPDATE runs SET dispatch_token='same-token' WHERE id=30")
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute("UPDATE runs SET dispatch_token='same-token' WHERE id=31")
    defaulted_id = conn.execute(
        "INSERT INTO runs(merchant_id,status,trigger_kind,created_at) "
        "VALUES (1,'running','manual','2026-09-04T00:03:00+00:00')"
    ).lastrowid
    assert conn.execute(
        "SELECT dispatch_state FROM runs WHERE id=?", (defaulted_id,)
    ).fetchone()[0] == "UNKNOWN"
    conn.close()


def test_fresh_run_omitting_dispatch_state_defaults_fail_closed_to_unknown(
    tmp_path, monkeypatch
):
    path = tmp_path / "fresh-run-dispatch-default.db"
    monkeypatch.setenv("SEO_OPS_DB", str(path))
    from app.db import init_db
    ...
    conn.execute(
        ...
        "VALUES (1,'Merchant','active','2026-09-04T00:00:00+00:00')"
    )
    run_id = conn.execute(
        "INSERT INTO runs(merchant_id,status,trigger_kind,created_at) "
        "VALUES (1,'running','manual','2026-09-04T00:01:00+00:00')"
    ).lastrowid

    assert conn.execute(
        "SELECT dispatch_state,coreai_run_id FROM runs WHERE id=?", (run_id,)
    ).fetchone() == ("UNKNOWN", None)
    conn.close()


def test_run_dispatch_reconciliation_is_one_per_run_and_append_only(
    tmp_path, monkeypatch
):
    path = tmp_path / "run-reconciliation.db"
    monkeypatch.setenv("SEO_OPS_DB", str(path))
    from app.db import init_db
    ...
    run_id = conn.execute(
        "INSERT INTO runs(merchant_id,status,trigger_kind,created_at) "
        "VALUES (1,'running','manual','2026-09-04T00:00:00+00:00')"
    ).lastrowid
    reconciliation_id = conn.execute(
        ...
        "result_dispatch_state,reason,created_at) "
        "VALUES (?,'NOT_CREATED',NULL,'operator','UNKNOWN','FAILED','not found',?)",
        (run_id, "2026-09-04T00:01:00+00:00"),
    ).lastrowid
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            "INSERT INTO run_dispatch_reconciliations("
            "run_id,action,operator_id,prior_dispatch_state,result_dispatch_state,"
            "reason,created_at) VALUES (?,'NOT_CREATED','operator','UNKNOWN',"
            "'FAILED','duplicate',?)",
            (run_id, "2026-09-04T00:02:00+00:00"),
        )
    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        conn.execute(
            "UPDATE run_dispatch_reconciliations SET reason='changed' WHERE id=?",
            (reconciliation_id,),
        )
    with pytest.raises(sqlite3.IntegrityError, match="append-only"):
        conn.execute(
            "DELETE FROM run_dispatch_reconciliations WHERE id=?",
            (reconciliation_id,),
        )
    conn.close()
