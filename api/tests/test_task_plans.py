...
def start_run(client, fake: FakeCoreAi):
    merchant = client.post(
        "/api/merchants",
        json={"name": "Plan merchant", "primary_location": "New York, NY"},
    ).json()
...
def test_agent_plan_persistence_is_idempotent_for_the_same_run_and_checksum(client):
    ...
    try:
        ...
        conn.execute(
            ...
            ("2026-09-04T00:01:00+00:00", run["id"]),
        )
...
def test_archived_merchant_rejects_every_plan_mutation_without_changes(client):
    from helpers import formal_workflow_snapshot

    merchant, _run, draft_plan = create_draft_plan(client)
    archived = client.patch(
        f"/api/merchants/{merchant['id']}", json={"status": "archived"}
    )
    assert archived.status_code == 200
    revision = draft_plan["current_revision"]
    ...
    responses = [
        ...
        client.post(
            f"/api/task-plans/{draft_plan['id']}/reject",
            json={"expected_revision": revision["revision"], "reason": "no"},
        ),
    ]
