...
- 诊断/Plan 入口优先使用 `COREAI_ORCHESTRATOR_AGENT_ID` 指定的 Core AI Agent；未配置或为空白时兼容 `COREAI_AGENT_ID`。手动和定时入口使用同一解析规则。未分配 Agent 的内容准备继续使用 `COREAI_PREPARATION_LLM_CALL_ID`；分配给 Agent 的任务只允许调用明确绑定的专用 Agent，配置见 [专用任务准备 Agent](docs/agents/task-preparation-binding.md)。分配不自动触发执行，结果仍需 AM 审批，不能自动发布。
