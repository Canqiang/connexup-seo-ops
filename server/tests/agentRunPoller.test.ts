...
describe("AgentRunPoller", () => {
  ...
  function seedRun(overrides: Partial<AgentRun> = {}): AgentRun {
    const run: AgentRun = {
  ...
  it("COMPLETED with artifacts: downloads attachments, failure keeps remote ref", async () => {
    ...
    expect(getAgentRun(db, run.id)?.status).toBe("COMPLETED");
    const deliverables = listDeliverablesByRun(db, run.id);
  ...
  it("second pollOnce after terminal is a no-op (crash-replay idempotence)", async () => {
    ...
    await poller.pollOnce();
    const once = listDeliverablesByRun(db, run.id);
    await poller.pollOnce();
    const twice = listDeliverablesByRun(db, run.id);
  ...
  it("FAILED: records error, no deliverables", async () => {
    ...
    expect(listDeliverablesByRun(db, run.id)).toHaveLength(0);
  });
  ...
  it("TRIGGERING orphan older than 60s fails as TRIGGER_INTERRUPTED", async () => {
    const run = seedRun({ status: "TRIGGERING", coreRunId: null });
    const poller = makePoller(
      {},
