...
日期：2026-09-08。状态：详细设计已确认；用户随后批准以专用 Agent 管理约定替代强制版本锁定。隔离分支实现、本地验收及用户批准的一次真实隔离 UAT 准备/审批闭环均已完成；主实例尚未绑定，代码尚未合并。验收记录见 `docs/agents/task-preparation-uat-acceptance-2026-09-08.md`。
...
本次业务代码仅修改 connexup-seo-ops。用户已另行批准创建专用 UAT 内容准备 Agent，范围仅此新 Agent，不修改现有 Agent 或 Core AI 服务代码。不运行真实任务，不改 SEO Ops 真实配置或数据库。实现使用独立 codex 分支，合并由用户另行选择。多用户、自动启动、GBP/官网发布、付费扫描、Plan revision 数据迁移均不在范围内。

## 2. 实施前代码基线（历史，不代表当前实现）
...
用户批准的简化：不要求接口锁定发布版本，改用专用 Agent ID 与不原地编辑/重发的管理约定；配置变更通过新建替代 Agent 完成，历史执行仍记录原 ID。保存已验证配置及校验和，可发现检查时的配置漂移；执行前后检查不等价于服务端强制版本锁定，检查与启动之间的变化窗口仍是明确接受的剩余风险。不能将本地快照描述成远端运行的不可变版本证明。
...
COMPLETED 只是远端状态：只有新协议结果通过现有结构化输出规范，并符合精确 run ID、Agent ID 和冻结请求关联，才保存 SUCCEEDED 和结果校验和、进入 AWAITING_APPROVAL。安全依据是已核验的无工具配置加专用 Agent 管理约定，不声称取得远端运行版本快照。展示摘要、产物引用、依据及远端运行 ID。外部引用只作为引用，不自动抓取或执行。
...
本地与 UAT 核查未取得远端执行版本绑定证据，用户已批准上述管理约定作为当前阶段替代方案；这个缺口不再单独阻塞本地实施。仍需明确配置漂移拒绝、身份核实和结构化输出校验；真实运行验收依然需单独授权。
