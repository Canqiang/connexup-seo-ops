...
- Every immutable Asset or Export owner receives its own write-once object key; different owners never share a key even when bytes are identical. A database row cannot become ready until reopened bytes match MIME, size, SHA-256, and format-specific validation.
- Asset metadata has an explicit fenced lifecycle `pending → ready|failed` and `pending|failed → deleting`. Ordinary orphan cleanup must first CAS an exact unreferenced row to `deleting`; publication rejects `deleting`, so bytes are never removed while another worker can still publish them ready. Merchant hard delete does not use this as a shortcut around durable-history guards.
...
- [ ] Write Asset Store tests for atomic temp write+fsync+rename, stable replay for one owner, different keys for two owners with identical content, collision with different bytes for one owner, interrupted write, reopen/hash/size validation, path traversal, missing file, MIME allowlist, concurrent writers, and cleanup of a crash-left staging object through its pending database owner.
- [ ] Use owner-exclusive keys such as `audit/<kind>/<owner-kind>/<owner-id>/g<lease-generation>.<allowlisted-extension>` and the deterministically derived sibling staging key for that exact lease. Build paths from validated components, open with no-follow/exclusive semantics where supported, and verify resolved paths remain below the configured root. A second Asset/Export owner must never reuse either key.
...
- [ ] Use `audit_assets` for both Evidence and Export objects with an exactly-one owner constraint (`RUN`, `LEGACY_SOURCE`, or `EXPORT_ATTEMPT`) and a unique Export Attempt owner. Before writing any durable byte, commit the pending Asset metadata with its owner-exclusive final key, deterministic staging key, and lease generation. Stream only to that staging key, atomically rename, then reopen and verify bytes before the Asset row can CAS from pending to ready. Thus a crash before metadata creates no durable bytes, while a crash after metadata leaves a database-tracked object that cleanup can resume. The CAS includes the current lease generation and `status='pending'`; `deleting` is terminal for that attempt and can never publish. Optional transport failure produces `AUDIT_ASSET_TRANSPORT_UNAVAILABLE`; a rubric-required attachment prevents Version acceptance.
...
- [ ] Claim and fence one Export Attempt in a short transaction. Render bytes outside the transaction but, before writing them durably, create its unique `owner_kind=EXPORT_ATTEMPT` pending `audit_assets` row and owner-exclusive keys under the current lease. Store, reopen, and validate bytes outside the transaction, CAS the Asset ready, then in one transaction verify the same Attempt/lease/ready Asset and publish the terminal Attempt plus Export `ready_asset_id`. A crash between render and metadata leaves no durable bytes; a crash after metadata leaves a cleanup-visible pending owner; a crash after Asset-ready is reconciled by the exact Attempt rather than starting another renderer.
...
- [ ] Implement a daily cleanup loop that claims one database-tracked pending/failed owner older than 24 hours in a short transaction: recheck no ready Asset/Export reference, CAS the exact row and lease generation to `deleting`, and commit. Delete only that owner's exclusive staging/final keys outside the transaction; then finalize metadata/event state only if the row is still `deleting`. A ready publisher must reject the deleting state. Accepted/ready content is never ordinary-cleanup eligible, and cleanup never touches another owner's key.
...
- [ ] Add tests for protected ready Version Asset, protected ready Export, young pending file, orphan pending file, two owners with identical bytes and distinct keys, publisher-versus-cleaner CAS race, publisher rejection after `deleting`, crash before pending metadata (no durable object), crash after pending metadata (cleanup-visible staging/final key), path traversal/collision, crash between file delete and event, and idempotent next pass.
