...
- Asset metadata has an explicit fenced lifecycle `pending → ready|failed` and `pending|failed → deleting`. Cleanup or merchant deletion must first CAS an exact unreferenced row to `deleting`; publication rejects `deleting`, so bytes are never removed while another worker can still publish them ready.
...
- [ ] Implement a daily cleanup loop that claims one database-tracked pending/failed object older than 24 hours in a short transaction: recheck no ready Asset/Export reference, CAS the exact row and lease generation to `deleting`, and commit. Delete only that verified internal object key outside the transaction; then finalize metadata/event state only if the row is still `deleting`. A ready publisher must reject the deleting state. Accepted/ready content is never ordinary-cleanup eligible.
- [ ] Extend explicit merchant hard delete for the no-Version/no-active-work case: in a short transaction recheck all merchant gates and CAS every exact unreferenced pending/failed Audit object to `deleting`; commit before deleting each verified object outside the transaction. A second transaction verifies all rows remain `deleting`, deletes their metadata, appends events, and calls Phase 1 `delete_unaccepted_audit_state`. Any CAS/reference change, ready Asset, or accepted Version returns a stable conflict and preserves the merchant; a later cleanup pass safely resumes bytes already fenced as deleting.
...
- [ ] Add tests for protected ready Version Asset, protected ready Export, young pending file, orphan pending file, publisher-versus-cleaner CAS race, publisher rejection after `deleting`, path traversal/collision, crash between file delete and event, and idempotent next pass.
