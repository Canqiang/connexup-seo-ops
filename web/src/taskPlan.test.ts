...
import { executionWaves, taskPlanValidationErrors, type TaskPlanItem } from './taskPlan'
...
describe('taskPlanValidationErrors', () => {
  it('rejects timezone-aware timestamps that omit RFC 3339 seconds', () => {
    expect(taskPlanValidationErrors([{
      ...task('draft'),
      scheduled_start: '2026-09-05T13:00Z',
    }])).toContain('任务 draft 的计划时间必须是带时区的 RFC 3339 时间')
  })

  it('accepts RFC 3339 timestamps with required seconds and an offset', () => {
    expect(taskPlanValidationErrors([{
      ...task('draft'),
      scheduled_start: '2026-09-05T13:00:00+08:00',
    }])).toEqual([])
  })
})
