  ...
  return (
    <li className="agent-workbench__history-item">
      ...
      <dl>
        ...
        <div><dt>完成</dt><dd>{item.completed_at ? <time dateTime={item.completed_at} aria-label={`完成时间 ${item.completed_at}`}>{item.completed_at}</time> : '未完成'}</dd></div>
        <div><dt>耗时</dt><dd>{item.duration_seconds === null ? '进行中' : `${item.duration_seconds} 秒`}</dd></div>
        ...
        <div><dt>关联</dt><dd>{item.association?.local_href ? <a href={item.association.local_href}>{item.association.local_label}</a> : '未关联商户'}</dd></div>
      </dl>
