...
function TaskDetailPage({ taskId }: { taskId: number }) {
  ...
  const beginAction = () => {
    if (!merchantActive || actionBusyRef.current) return null
  ...
  const cancelTask = async () => {
    if (!merchantActive || !task || !cancelReason.trim()) return
    const controller = beginAction()
    if (!controller) return
    try {
  ...
  const retryPreparation = async () => {
    if (!merchantActive || !task || !retryReason.trim()) return
    const controller = beginAction()
    if (!controller) return
    try {
  ...
  const returnForRevision = async () => {
    if (!merchantActive || !task || !returnReason.trim()) return
    const execution = latestExecution(task)
    if (!execution) return
    const controller = beginAction()
    if (!controller) return
  ...
  const canExecute = merchantActive
    && task.status === 'PENDING'
    && task.readiness === 'READY'
    && task.replaced_by_task_id === null
    && !task.executions.some(isActiveExecution)
  const canCancel = merchantActive
    && ['PENDING', 'AWAITING_APPROVAL', 'NEEDS_ATTENTION'].includes(task.status)
    && !task.executions.some(isActiveExecution)
  const canRetry = merchantActive && isRetryablePreparation(task, latest)
  const latestTrust = trustedPreparation(task, latest)
  const canReview = merchantActive && latestTrust.kind === 'reviewable'
              ...
              <div className="task-label-field">
                ...
                <div className="task-label-editor" role="group" aria-label="内部标签编辑器">
                  ...
                  <button type="button" className="quiet" aria-label="新增内部标签" onClick={() => setLabels(current => {
                    ...
                    labelsDraftRef.current = next
                    return next
                  })} disabled={!merchantActive || busy || labels.length >= 20}>＋ 添加标签</button>
                </div>
                <small>逐项编辑，保存时自动 trim / 去重，最多 20 个。</small>
              </div>
              <label>操作人备注<textarea aria-label="操作人备注" maxLength={2000} value={operatorNote} onChange={event => { operatorNoteDraftRef.current = event.target.value; setOperatorNote(event.target.value) }} disabled={!merchantActive || busy} /></label>
            </div>
